...
for i in range(len(files)):
    ...
    # 获取列表长度
    n=0
    for m in range(len(addr3) - 1):
        n = m
        if n < len(addr3) - 1 and addr3[m] < addr3[n + 1]:
            # j=j+1
            addr2[m + 1] = int(addr2[m]) * (int(addr2[m + 1])+1)
            n = n + 1
            if n < len(addr3) - 2 and addr3[m] < addr3[n + 1]:
                addr2[n + 1] = int(addr2[m]) * (int(addr2[n + 1])+1)
...
# 在235ns
    matched_strings_235 = []
    for line_1 in lines:
        # 使用条件判断行中是否包含"235ns"和"rtl.v"
        if "235 ns" in line_1 and "rtl.v" in line_1:
            # 使用split方法以"Line"作为分隔符拆分行
            parts = line_1.split("Line")
            if len(parts) > 1:
                # 提取分隔后的第二部分，并去除前后的空格
                matched_string = parts[1].strip()
                # 将匹配的字符串添加到列表中
...
# 在245ns
    ...
    for line_2 in lines:
        # 使用条件判断行中是否包含"225ns"和"rtl.v"
        if "245 ns" in line_2 and "rtl.v" in line_2:
            # 使用split方法以"Line"作为分隔符拆分行
            parts = line_2.split("Line")
            if len(parts) > 1:
                # 提取分隔后的第二部分，并去除前后的空格
                matched_string = parts[1].strip()
                # 将匹配的字符串添加到列表中
